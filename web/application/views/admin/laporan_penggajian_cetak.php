...
<table width="100%"  border="1" style="font-size: 12pt; padding: 4px">
    <tr>
        <th class="text-center" width="70%"><b>Jenis Gaji</b></th>
        <th class="text-center" width="30%"><b>Nominal</b></th>
    </tr>';
...
foreach ($dtkaryawan as $row) {
    ...
    $html .= '
        <tr style="background-color: #e1e1e1">
            <td colspan="2"><b>Nama : ' . $row->nama_karyawan . '</b> </td>
        </tr>';

    if ($row->total_gaji != "") {
        foreach ($row->total_gaji as $jenis_gaji) {
            $total_per[] = $jenis_gaji->nominal_gaji;
            $html .= '
                <tr>
...
$html .= '<tr style="background-color: #e1e1e1">
                <td><b>Total Pengeluaran Gaji : </b></td>
                <td>Rp. '.number_format(array_sum($total_per)) .'</td>
            </tr>
</table>';
